...
const GradualBlur: React.FC<GradualBlurProps> = props => {
  ...
  const blurDivs = useMemo(() => {
    ...
    for (let i = 1; i <= config.divCount; i++) {
      ...
      // Membuat gradien yang lebih halus dengan tumpang tindih
      const start = Math.max(0, (i - 1.5) * increment);
      const end = Math.min(100, (i + 0.5) * increment);
      const gradient = `transparent ${start}%, black ${end}%`;
